const fs = require("fs");
const { flatten, zipWith, compact } = require("lodash/array");
const { exec, splitLines, splitByWhitespace } = require("./utility");
const { isEmpty } = require("lodash/lang");
const { map, filter } = require("lodash/collection");
const { statementList, digraph } = require("./dot_builders");
const {
    mapRawCommit,
    mapRawTree,
    mapAnnotatedTag,
...
(async function () {
    ...
    const tagData = map(compact(splitLines(await showTags())), mapLineToRef);

    const statements = [
        ...map(blobs, mapBlobToStatement),
        ...flatten(map(commitData, mapCommitToStatements)),
        ...flatten(map(treeData, mapTreeToStatements)),
        ...flatten(map(headData, mapHeadToStatements)),
        ...flatten(mapHeadToStatements({ hash: HEAD, name: "HEAD" })),
        ...flatten(map(annotatedTagData, mapAnnotatedTagsToStatements)),
        ...flatten(map(tagData, mapTagToStatements)),
    ];

    console.log(digraph("g", statementList(statements)));
})();
